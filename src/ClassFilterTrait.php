...
trait ClassFilterTrait
{
    ...
    public function getFilteredClasses($row): array
    {
        ...
        if (! isset($this->classfilter)) {
            return $classes;
        }

        foreach ($this->classfilter as $filter) {
            [$operand_a, $operator, $operand_b] = $filter->conditions;
            ...
            if (in_array($this->formattype, ['int', 'number', 'float', 'decimal'], true)) {
                $field = (string) $field;
                $value = (string) $value;
            }
            ...
            switch ($operator) {
                ...
                case 'in':
                    if (in_array($field, explode(',', $filter->value), true)) {
                        $classes[] = $filter->class;
                    }
                    break;
